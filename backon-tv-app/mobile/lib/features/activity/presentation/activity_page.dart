...
Future<void> _showConfirmEventDialog({
  ...
  await AnalyticsService.popupOpen(popupName: 'koniec_reklam_confirm');
  final startedAt = DateTime.now();
  final result = await EventConfirmationDialog.showWithAdPreload(
    context: context,
    title: title,
  );
  final visibleSeconds = DateTime.now().difference(startedAt).inSeconds;
  await AnalyticsService.popupVisibleTime(popupName: 'koniec_reklam_confirm', seconds: visibleSeconds);

  if (result != null) {
    await AnalyticsService.koniecReklamClicked(programId: programId, source: 'confirm');
