...
Future<void> _showCreateEventDialog({
  ...
  await AnalyticsService.popupOpen(popupName: 'koniec_reklam_create');
  ...
  await AnalyticsService.popupVisibleTime(popupName: 'koniec_reklam_create', seconds: visibleSeconds);

  if (confirmed == true) {
    await AnalyticsService.koniecReklamClicked(programId: programId, source: 'schedule');
    try {
      await onConfirm();
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
