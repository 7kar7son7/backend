...
export function startEpgImportJob(app: FastifyInstance): ScheduledTask | null {
  ...
  if (!enabled) {
    app.log.info(
      'EPG auto-import job disabled (EPG_AUTO_IMPORT_ENABLED=false lub brak AKPA i domyślne wyłączenie).',
    );
    return null;
  }
  ...
  const maxAgeDaysForPrune = Math.max(
    1,
    Number.parseInt(env.EPG_PRUNE_MAX_AGE_DAYS ?? process.env.EPG_PRUNE_MAX_AGE_DAYS ?? '14', 10) || 14,
  );

  /** Wspólna logika importu – używana przez cron i import startowy (jeden mutex `isRunning`). */
  async function runImportPipeline(source: 'cron' | 'startup'): Promise<void> {
    if (!useAkpa) {
      try {
        ...
        const grabMsg =
          source === 'cron'
            ? 'Aktualizacja feedu (grab) nie powiodła się, kontynuuję import z istniejących danych.'
            : 'Aktualizacja feedu (grab) na starcie nie powiodła się, używam bieżącego pliku.';
        app.log.error(error, grabMsg);
      }
    ...
    if (useAkpa) {
      ...
      await epgService.pruneOldPrograms(maxAgeDaysForPrune);
    }
  }
  ...
  if (runOnStart) {
    ...
    setTimeout(async () => {
      if (isRunning) {
        app.log.warn(
          'Initial EPG import skipped: another import is already running (np. równoległy tick crona).',
        );
        return;
      }
      isRunning = true;
      try {
        ...
        await runImportPipeline('startup');
